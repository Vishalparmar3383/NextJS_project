...
type Book = {
    ...
    title: string;
    author: string;
    isbn?: string; // Added since it's in your data
...
export default function BooksList() {
        ...
        async function loadBooks() {
            try {
                ...
                if (data.success && data.books) {  
                    // Debug: Log the first book to check its structure
                    console.log('First book data:', data.books[0]);
                    console.log('All book IDs:', data.books.map(book => ({ title: book.title, id: book.id })));
                    setBooks(data.books);
                } else {
    ...
    // Helper function to get the book ID safely
    const getBookId = (book: Book) => {
        return book.id; // Simplified since we know the field name is 'id'
    };

    // Calculate total available copies across all books
    const totalAvailableCopies = books.reduce((total, book) => total + book.availableCopies, 0);
                ...
                <div className="max-w-7xl mx-auto">
                    <div className="flex items-center justify-center min-h-[400px]">
                        <div className="text-center">
                            ...
                            <h3 className="text-xl font-semibold text-slate-800 mb-2">Failed to load books</h3>
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-slate-700">Manage your book inventory</p>
                            </div>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                        <div className="bg-white/70 backdrop-blur-sm rounded-2xl p-6 border border-white/20 shadow-lg">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="text-slate-700 text-sm font-medium">Total Books</p>
                                    <p className="text-2xl font-bold text-slate-900">{books.length}</p>
                                </div>
                            </div>
                        </div>
                        
                        {/* Available Copies - FIXED */}
                        <div className="bg-white/70 backdrop-blur-sm rounded-2xl p-6 border border-white/20 shadow-lg">
                            <div className="flex items-center gap-3">
                        ...
                        <div className="bg-white/70 backdrop-blur-sm rounded-2xl p-6 border border-white/20 shadow-lg">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-2xl font-bold text-slate-900">
                                        {books.reduce((total, book) => total + book.totalCopies, 0)}
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="bg-white/70 backdrop-blur-sm rounded-2xl p-6 border border-white/20 shadow-lg mb-6">
                        <div className="flex flex-col md:flex-row gap-4">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <select
                                    value={sortBy}
                                    onChange={(e) => setSortBy(e.target.value as 'title' | 'author' | 'genre' | 'year' | 'status')}
                ...
                {filteredBooks.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {filteredBooks.map(book => {
                            ...
                            return (
                                <div key={bookId} className="group bg-white/70 backdrop-blur-sm rounded-2xl p-6 border border-white/20 shadow-lg hover:shadow-xl transform hover:scale-[1.02] transition-all duration-200">
                                    <div className="flex flex-col h-full">
                                        <div className="flex-1">
                                            <div className="flex items-start justify-between mb-3">
                                                ...
                                                <div className="text-xs text-gray-400">ID: {bookId}</div>
                                            </div>
                                            ...
                                            <div className="space-y-2 mb-4">
                                                ...
                                                <div className="text-xs text-slate-500 space-y-1">
                                                    ...
                                                    <div className="flex justify-between">
                                                        <span>Issued:</span>
                                                        <span className="text-red-600">{book.issuedCopies}</span>
                                                    </div>
                                                </div>
                                                {book.genre && (
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="bg-slate-50/50 border-b border-slate-200">
                                        <th className="text-left py-4 px-6 font-semibold text-slate-800">Book Title</th>
                                ...
                                <tbody>
                                    {filteredBooks.map((book) => {
                                        ...
                                        return (
                                            <tr key={bookId} className="border-b border-slate-100 last:border-b-0 hover:bg-slate-50/50 transition-colors duration-200">
                                                ...
                                                <td className="py-4 px-6 text-slate-800 font-medium">{book.author}</td>
                                                <td className="py-4 px-6 text-slate-600">{book.genre || '-'}</td>
                                                <td className="py-4 px-6 text-center text-slate-600">{book.year || '-'}</td>
                                                ...
                                                <td className="py-4 px-6 text-center">
                                                    <div className="text-sm text-slate-700">
                                                        <div className="font-medium">{book.totalCopies}</div>
                                                        <div className="text-xs text-slate-500">
                                                            <span className="text-green-600">{book.availableCopies} available</span>
                                                            <br />
                                                            <span className="text-red-600">{book.issuedCopies} issued</span>
                                                        </div>
                                                    </div>
                                                </td>
                                                ...
                                                <td className="py-4 px-6">
                                                    <div className="flex items-center justify-center gap-2">
                                                        <Link
                                                            href={`/librarian/books/${bookId}`}
                                                            className="flex items-center gap-1 px-3 py-1.5 bg-indigo-50 text-indigo-700 rounded-lg hover:bg-indigo-100 transition-colors duration-200 text-sm font-medium"
                                                        >
                                                            <Eye className="w-4 h-4" />
                                                            View
                                                        </Link>
                                                        <Link
                                                            href={`/librarian/books/${bookId}/edit`}
                                                            className="flex items-center gap-1 px-3 py-1.5 bg-green-50 text-green-700 rounded-lg hover:bg-green-100 transition-colors duration-200 text-sm font-medium"
                                                        >
